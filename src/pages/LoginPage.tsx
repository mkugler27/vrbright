  ...
  const handleLogin = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (result.status === 'success' || result.success) {
        const idBubble = result.user?.id_bubble || result.response?.user_id || '';
        const nome = result.user?.nome || email.split('@')[0];
        const token = result.response?.token || result.token || '';
        const expires = result.response?.expires || 0;
        const pictureFromLogin = result.user?.profile_picture;
        ...
        setUser({
          ...
          token,
          expires,
          profile_picture: pictureFromLogin,
        });

        // Fetch full profile (including profile_picture) from Bubble
        if (token && idBubble) {
          try {
            const profile = await fetchUserProfile(idBubble, token);
            setUser({
              id_bubble: profile.id_bubble || idBubble,
              nome: profile.nome || nome,
              email: profile.email || email,
              token,
              expires,
              profile_picture: profile.profile_picture || pictureFromLogin,
            });
          } catch (profileErr) {
            console.warn('Could not fetch full profile, continuing with login data:', profileErr);
          }
        }
