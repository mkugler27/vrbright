...
export function AdjustmentPage() {
  ...
  const touchCurrentOffsetRef = useRef<number>(0);
  const activeSwipeElementRef = useRef<HTMLDivElement | null>(null);

  const handleTouchStart = (e: React.TouchEvent, id: string, disabled: boolean) => {
    if (disabled) return;
    const touch = e.touches[0];
    touchStartRef.current = { x: touch.clientX, y: touch.clientY };
    touchCurrentOffsetRef.current = 0;
    
    // Close other swiped cards
    if (swipedId && swipedId !== id) {
      const activeEl = document.getElementById(`swipe-fg-${swipedId}`);
      if (activeEl) {
  ...
  const handleTouchMove = (e: React.TouchEvent, id: string, disabled: boolean) => {
    ...
    if (Math.abs(diffX) > Math.abs(diffY)) {
      ...
      activeSwipeElementRef.current = el;
      
      // Calculate translation (only allow swiping left, diffX < 0)
      let translation = 0;
      if (diffX < 0) {
        // Safe cap of -80px
        translation = Math.max(-80, diffX);
      }
      
      touchCurrentOffsetRef.current = translation;
      el.style.transform = `translateX(${translation}px)`;
      el.style.transition = 'none';
    }
  };

  const handleTouchEnd = (e: React.TouchEvent, id: string, disabled: boolean) => {
    if (disabled || !touchStartRef.current) return;
    
    const el = activeSwipeElementRef.current || (e.currentTarget as HTMLDivElement);
    el.style.transition = 'transform 0.2s cubic-bezier(0.16, 1, 0.3, 1)';
    
    const finalOffset = touchCurrentOffsetRef.current;
    ...
    activeSwipeElementRef.current = null;
  };
                  ...
                  <div key={adj.id} className="relative overflow-hidden rounded-[24px]">
                    ...
                    {canDelete && (
                      <button
                        ...
                        className="absolute right-0 top-0 bottom-0 w-[75px] bg-red-600 rounded-[24px] flex items-center justify-center text-white active:bg-red-700 transition-colors"
                        title="Delete"
                      >
