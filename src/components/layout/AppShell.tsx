...
const BOTTOM_NAV = [
  ...
  { to: '/wo', label: 'Works', icon: <WorksIcon /> },
  ...
  return (
    <div className="flex flex-col h-full bg-gray-50">
      ...
      <header className="bg-gradient-to-r from-primary-dark to-primary px-5 py-4 flex items-center justify-between sticky top-0 z-20 shadow-md">
        ...
        {/* Right: Name + Photo + Online indicator */}
        <div className="flex items-center gap-3">
          <div className="text-right min-w-0">
            <p className="text-sm font-semibold text-white truncate max-w-[140px]">{user?.nome}</p>
            {user?.email && (
              <p className="text-[10px] text-white/70 truncate max-w-[140px]">{user.email}</p>
            )}
          </div>
          <span
      ...
      {menuOpen && (
        <div className="fixed inset-0 z-30 flex">
          <div
            className="w-72 bg-white shadow-2xl flex flex-col animate-slideIn"
          >
            {/* User info */}
            ...
            <div className="flex-1 overflow-y-auto p-3">
              <p className="text-[10px] uppercase tracking-wide text-gray-400 font-semibold px-3 py-2">
                Main
              </p>
              ...
              <p className="text-[10px] uppercase tracking-wide text-gray-400 font-semibold px-3 py-2 mt-3">
                Other
              </p>
              {MENU_ITEMS.filter((i) => !i.primary).map((item) => (
                <button
