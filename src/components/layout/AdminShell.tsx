...
export function AdminShell() {
  ...
  const menuItems = [
    ...
    {
      to: '/admin/client-prices',
      label: 'Client Prices',
      icon: (
        <svg className="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
          <path strokeLinecap="round" strokeLinejoin="round" d="M7 7h.01M6 20h12a2 2 0 002-2V8a2 2 0 00-2-2H6a2 2 0 00-2-2H4a2 2 0 00-2 2v10a2 2 0 002 2zM9 16H5v2h4v-2zm7-5h-4v2h4v-2zm-7 0H5v2h4v-2z" />
        </svg>
      ),
    },
    {
          ...
          {menuItems.map((item) => {
            const isActive =
              ...
              (item.to !== '/admin' &&
                ...
                !(item.to === '/admin/proposals' && (
                  location.pathname.startsWith('/admin/proposals/templates') ||
                  location.pathname.startsWith('/admin/proposals/details')
                )));
            return (
              <NavLink
                key={item.to}
